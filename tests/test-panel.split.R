...
single <- panel.split(prepared, initial = 6, horizon = 2)
stopifnot(length(single$folds) == 1L, identical(single$folds[[1]]$validation_period, 2006:2007))
stride <- panel.split(prepared, initial = 3, horizon = 2, step = 2)
...
large_step <- panel.split(prepared, initial = 3, horizon = 1, step = 100)
stopifnot(length(large_step$folds) == 1L)
gaps <- panel.split(prepared, initial = 3, horizon = 1, step = 2)
...
spaced <- panel.dataprep(spaced_panel, "state", "year", "sales", "Target",
                        ...
                        period_step = 2)
spaced_splits <- panel.split(spaced, 4, 2)
...
stopifnot(identical(splits, panel.split(prepare(panel[rev(seq_len(nrow(panel))), ]), 4, 2)))
changed <- panel
changed$sales <- changed$sales + 100
changed$sales[changed$year >= 2008] <- NA_real_
stopifnot(identical(splits, panel.split(prepare(changed), 4, 2)),
          identical(splits, panel.split(prepare(panel[panel$year < 2008, ]), 4, 2)))

# Reject missing, nonscalar, nonnumeric, nonfinite and infeasible window inputs.
expect_error(panel.split(prepared, horizon = 2), "supplied explicitly")
expect_error(panel.split(prepared, initial = 4), "supplied explicitly")
for (bad in list(NULL, NA_real_, NaN, Inf, -1, 0, 1.5, "4", TRUE, c(3, 4), matrix(4))) {
  expect_error(panel.split(prepared, bad, 2), "initial must be one positive")
  expect_error(panel.split(prepared, 4, bad), "horizon must be one positive")
  expect_error(panel.split(prepared, 4, 2, bad), "step must be one positive")
}
expect_error(panel.split(prepared, 2, 2), "at least three")
expect_error(panel.split(prepared, 8, 1), "No full fold fits")
expect_error(panel.split(prepared, 4, 5), "No full fold fits")
short <- panel.dataprep(panel, "state", "year", "sales", "Target",
                       c("DonorB", "DonorA"), 2008, 2000:2002, "sales")
expect_error(panel.split(short, 3, 1), "No full fold fits")

# Reject unsupported object versions and invalid date metadata.
expect_error(panel.split(list(), 4, 2), "tvsc_prepared")
invalid <- prepared
invalid$schema_version <- 2L
expect_error(panel.split(invalid, 4, 2), "schema-version-1")
invalid <- prepared
invalid$schema$donors <- NULL
expect_error(panel.split(invalid, 4, 2), "incomplete or ambiguous schema")
invalid <- prepared
invalid$schema$pre_period <- rev(invalid$schema$pre_period)
expect_error(panel.split(invalid, 4, 2), "regular pre-treatment dates")
invalid <- prepared
