...
named_predictors <- tvsc.blocks(prepare(predictors = c(label = "income", other = "sales")), 2003)
...
stopifnot(identical(blocks, tvsc.blocks(reordered, 2003)))
...
stopifnot(identical(blocks, tvsc.blocks(changed, 2003)))
changed$training <- changed$training[!later, ]
stopifnot(identical(blocks, tvsc.blocks(changed, 2003)))

# Support both the complete pre-period and a cutoff returned by tvsc.split().
full <- tvsc.blocks(prepared, tail(prepared$schema$pre_period, 1))
...
stopifnot(identical(blocks, tvsc.blocks(prepared, splits$folds[[1]]$cutoff)))

# Do not add the outcome to predictor blocks when it is explicitly omitted.
without_outcome <- tvsc.blocks(prepare(predictors = "income"), 2003)
...
outcome_only <- tvsc.blocks(prepare(predictors = "sales"), 2003)
...
transformed_outcome <- tvsc.dataprep(
  scaled_panel, "state", "year", "sales_scaled", "Target", c("DonorB", "DonorA"),
  2006, 2000:2005, "income_scaled"
)
transformed_blocks <- tvsc.blocks(transformed_outcome, 2003)
...
numeric_blocks <- tvsc.blocks(numeric_prepared, 2002)
...
stopifnot(identical(blocks, tvsc.blocks(prepare(factor_panel), 2003)))
spaced_panel <- panel
...
stopifnot(identical(tvsc.blocks(spaced, 2004)$periods, c(2000, 2002, 2004)))
expect_error(tvsc.blocks(spaced, 2005), "must match a date")

# Fail explicitly for invalid cutoffs and malformed object metadata.
expect_error(tvsc.blocks(prepared), "supplied explicitly")
for (bad in list(NULL, NA_real_, NaN, Inf, "2003", TRUE, c(2002, 2003), 2003.5, matrix(2003))) {
  expect_error(tvsc.blocks(prepared, bad), "finite integer-valued numeric pre-period date")
}
expect_error(tvsc.blocks(prepared, 2001), "at least three")
expect_error(tvsc.blocks(prepared, 2006), "must match a date")
expect_error(tvsc.blocks(prepared, 4), "must match a date")
expect_error(tvsc.blocks(list(), 2003), "tvsc_prepared")
invalid <- prepared
invalid$schema_version <- 2L
expect_error(tvsc.blocks(invalid, 2003), "schema-version-1")
invalid <- prepared
invalid$schema$donors <- NULL
expect_error(tvsc.blocks(invalid, 2003), "incomplete or ambiguous schema")
invalid <- prepared
invalid$schema$pre_period <- rev(invalid$schema$pre_period)
expect_error(tvsc.blocks(invalid, 2003), "regular pre-treatment dates")
invalid <- prepared
invalid$recipe$scaling <- "standardize"
expect_error(tvsc.blocks(invalid, 2003), "contemporaneous raw recipe")

# Revalidate required prefix keys and values, including a non-predictor outcome.
invalid <- prepared
invalid$training <- invalid$training[-1, ]
expect_error(tvsc.blocks(invalid, 2003), "Missing training key")
invalid <- prepared
invalid$training <- rbind(invalid$training, invalid$training[1, ])
expect_error(tvsc.blocks(invalid, 2003), "Duplicate training key")
invalid <- prepared
invalid$training$income[1] <- NA_real_
expect_error(tvsc.blocks(invalid, 2003), "Nonfinite training value")
invalid <- prepare(predictors = "income")
invalid$training$sales[1] <- Inf
expect_error(tvsc.blocks(invalid, 2003), "Nonfinite training value")
invalid <- prepared
invalid$training$income <- factor(invalid$training$income)
expect_error(tvsc.blocks(invalid, 2003), "must be numeric")
