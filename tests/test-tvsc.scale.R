...
prepare <- function(data = panel, matching = predictors) {
  tvsc.dataprep(data, "state", "year", "sales", "Target", c("DonorB", "DonorA"),
...
stopifnot(identical(before, serialize(blocks, NULL)),
          ...
          identical(scaled, tvsc.scale(blocks, "sd")))
...
custom <- tvsc.scale(blocks, "custom", custom_scales)
for (date_index in seq_along(blocks$periods)) {
...
for (processed in list(scaled, zscored, unchanged, custom)) {
  expect_error(tvsc.scale(processed), "Already processed")
}
forged <- scaled
class(forged) <- "tvsc_blocks"
expect_error(tvsc.scale(forged), "Already processed")
...
shift_blocks <- tvsc.blocks(prepare(shifted), 2003)
shift_sd <- tvsc.scale(shift_blocks)
shift_zscore <- tvsc.scale(shift_blocks, "zscore")
...
stopifnot(identical(scaled, tvsc.scale(tvsc.blocks(later, 2003))))
...
single <- tvsc.blocks(prepare(matching = "signal"), 2003)
for (method in c("sd", "zscore", "none", "custom")) {
  result <- tvsc.scale(single, method, if (method == "custom") c(signal = 20) else NULL)
...
pre_blocks <- tvsc.blocks(prepare(pre_scaled), 2003)
stopifnot(identical(tvsc.scale(pre_blocks, "none")$X1, pre_blocks$X1),
          identical(tvsc.scale(pre_blocks)$Y0, blocks$Y0))
numeric_panel <- panel
numeric_panel$state <- match(numeric_panel$state, c("Target", "DonorA", "DonorB"))
numeric_prepared <- tvsc.dataprep(numeric_panel, "state", "year", "sales", 1, c(3, 2),
                                  2005, 2000:2004, "signal")
numeric_scaled <- tvsc.scale(tvsc.blocks(numeric_prepared, 2003))
stopifnot(identical(numeric_scaled$recipe$scaling$reference$donors, c(3, 2)),
          identical(colnames(numeric_scaled$X0[[1]]), c("3", "2")))
named_blocks <- tvsc.blocks(prepare(matching = c(alias = "signal")), 2003)
stopifnot(identical(rownames(tvsc.scale(named_blocks)$recipe$scaling$divisors), "signal"))
...
expect_error(tvsc.scale(underflow), "Numerical SD failure")
overflow <- tiny
overflow$X1[[1]][] <- -1e200
overflow$X0[[1]][, ] <- c(1e200, 0)
expect_error(tvsc.scale(overflow), "Numerical SD failure")
stopifnot(identical(tvsc.scale(overflow, "none")$X1, overflow$X1))
expect_error(tvsc.scale(single, "custom", c(signal = 1e-320)), "Nonfinite transformed")
...
for (bad in list(NULL, NA_character_, "s", "z-score", "mad", "iqr", "minmax", "maxabs",
  ...
  expect_error(tvsc.scale(blocks, bad), "scaling must be exactly")
}
for (bad in list(NULL, rep(1, 4), c(signal = 1),
  ...
  expect_error(tvsc.scale(blocks, "custom", bad), "custom scales must be")
}
for (method in c("sd", "zscore", "none")) {
  expect_error(tvsc.scale(blocks, method, custom_scales), "scales must be NULL")
}

# Validate schema, date alignment, recipes, shapes and finite values at entry.
expect_error(tvsc.scale(list()), "schema-version-1")
invalid <- blocks
invalid$schema_version <- 2L
expect_error(tvsc.scale(invalid), "schema-version-1")
invalid <- blocks
invalid$schema$donors <- NULL
expect_error(tvsc.scale(invalid), "incomplete or ambiguous")
invalid <- blocks
invalid$periods <- rev(invalid$periods)
expect_error(tvsc.scale(invalid), "regular pre-treatment")
invalid <- blocks
invalid$cutoff <- 2002
expect_error(tvsc.scale(invalid), "regular pre-treatment")
invalid <- blocks
...
expect_error(tvsc.scale(invalid), "contemporaneous raw recipe")
invalid <- blocks
invalid$dimensions$donors <- 3L
expect_error(tvsc.scale(invalid), "dimensions must agree")
invalid <- blocks
names(invalid$X1) <- rev(names(invalid$X1))
expect_error(tvsc.scale(invalid), "date-named lists")
invalid <- blocks
invalid$X0[[1]] <- invalid$X0[[1]][, 2:1, drop = FALSE]
expect_error(tvsc.scale(invalid), "Invalid finite numeric predictor")
invalid <- blocks
invalid$X1[[1]][1] <- NA_real_
expect_error(tvsc.scale(invalid, "none"), "Invalid finite numeric predictor")
invalid <- blocks
invalid$Y0[1, 1] <- Inf
expect_error(tvsc.scale(invalid), "finite numeric outcomes")
invalid <- blocks
